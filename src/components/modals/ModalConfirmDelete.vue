...
const open = (data) => {
    user.value = data
    confirmButtonRef.value?.click()
}
const emits = defineEmits(['confirmed'])
const confirm = (id) => {
  emits('confirmed', id)
  triggerCancelButton.value?.click() 
}
...
<template>
    <button type="button" ref="confirmButtonRef" :class="{ 'hidden': isHiddenTriggerButton }"
        class="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out"
        data-bs-toggle="modal" data-bs-target="#modalConfirmDelete">Confirm</button>

    <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
        id="modalConfirmDelete" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            ...
            <div
                ...
                <div class="modal-body relative p-4">
                    <div class="flex items-center">
                        Are you sure you want to delete this user "<span class="text-blue-600">{{ user?.name }}</span>"?
                    </div>
                </div>
                <div
                    class="modal-footer flex flex-shrink-0 flex-wrap items-center justify-end p-4 border-t border-gray-200 rounded-b-md">
                    <button type="button" ref="triggerCancelButton"
                    ...
                    <button type="button" @click="confirm(user.id)"
